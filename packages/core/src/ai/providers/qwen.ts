 ...
 * 支持 qwen-vl-max 等 Vision 模型
 */

import type { VideoInfo, VideoAnalysis, ReplyStyle } from '@douyinclaw/shared';
import type { QwenConfig, TokenUsage, ImageData, ChatHistory, LLMCapabilities } from '../types.js';
import { LLMError } from '../types.js';
import { BaseLLMProvider } from './base.js';

// Qwen 默认配置
const DEFAULT_MODEL = 'qwen-vl-max'; // 支持视觉的模型
const DEFAULT_BASE_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1';

/**
 * Qwen Provider 能力
 */
const QWEN_CAPABILITIES: LLMCapabilities = {
  vision: true,
  streaming: true,
...
export class QwenProvider extends BaseLLMProvider {
  ...
  constructor(config: QwenConfig = { provider: 'qwen' }) {
    super(config);

    this.apiKey = config.apiKey ?? process.env.QWEN_API_KEY;
    this.baseUrl = config.baseUrl ?? process.env.QWEN_BASE_URL ?? DEFAULT_BASE_URL;
    this._model = config.model ?? process.env.QWEN_MODEL ?? DEFAULT_MODEL;
  }
...
请生成一条简短、温馨的日常问候消息（10-30字）。只返回消息文字。`;
  ...
  private mockAnalyzeVideo(videoInfo: VideoInfo): { analysis: VideoAnalysis; tokens: TokenUsage } {
    return {
      analysis: {
        summary: `[Qwen] 这是一个关于${videoInfo.title || '未知内容'}的视频`,
        mood: 'neutral',
  ...
  private mockGenerateReply(
    ...
    const replies: Record<ReplyStyle['tone'], string> = {
      casual: `[Qwen] 这个视频挺有意思的，${analysis.summary}`,
      humorous: `[Qwen] 哈哈，这个视频绝了！${analysis.summary}`,
      analytical: `[Qwen] 看了这个视频，我觉得${analysis.summary}`,
      brief: '[Qwen] 不错！',
    };
  ...
  private mockGenerateSparkMessage(friendName: string): { message: string; tokens: TokenUsage } {
    const messages = [
      `[Qwen] ${friendName}，今天怎么样？`,
      `[Qwen] 嘿 ${friendName}，最近忙啥呢？`,
      `[Qwen] ${friendName}，天气不错哦`,
    ];
...
export function createQwenProvider(config?: Partial<QwenConfig>): QwenProvider {
  return new QwenProvider({ provider: 'qwen', ...config });
}
